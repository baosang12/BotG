...
{
    public class ReversalBlockDetector : ISmartMoneyDetector
    {
        private readonly BotConfig _cfg;
        public ReversalBlockDetector(BotConfig config) { _cfg = config; IsEnabled = _cfg.ShowReversalBlocks; }
        public SmartMoneyType Type => SmartMoneyType.ReversalBlock;
        public bool IsEnabled { get; set; }

        public bool Detect(IList<Bar> bars, out SmartMoneySignal signal)
        {
            signal = null;
            if (!IsEnabled || bars == null || bars.Count < _cfg.ReversalBlockLookback) return false;
            int look = Math.Min(_cfg.ReversalBlockLookback, bars.Count - 1);
