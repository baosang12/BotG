...
{
    public class MitigationBlockDetector : ISmartMoneyDetector
    {
        private readonly BotConfig _cfg;
        public MitigationBlockDetector(BotConfig config) { _cfg = config; IsEnabled = _cfg.ShowMitigationBlocks; }
        public SmartMoneyType Type => SmartMoneyType.MitigationBlock;
        public bool IsEnabled { get; set; }

        public bool Detect(IList<Bar> bars, out SmartMoneySignal signal)
        {
            signal = null;
            if (!IsEnabled || bars == null || bars.Count < _cfg.MitigationBlockLookback) return false;
            int look = Math.Min(_cfg.MitigationBlockLookback, bars.Count - 1);
